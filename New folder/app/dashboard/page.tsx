...
import { UserMenu } from "@/components/user-menu" // Assuming this component exists and is correct
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } from "recharts"
import { Upload, DollarSign, TrendingUp, FileText, Calendar, Eye, Lock, ShoppingCart } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (datasetsRes.ok) {
        ...
        // Ensure datasetsData is an array before setting state
        setDatasets(Array.isArray(datasetsData) ? datasetsData : [])
      }

      if (purchasesRes.ok) {
        const purchasesData = await purchasesRes.json()
        // Ensure purchasesData is an array before setting state
  ...
  // Ensure datasets is an array before calling reduce
  const categoryData = Array.isArray(datasets)
    ? datasets.reduce(
        (acc, dataset) => {
    ...
    : {} // Default to an empty object if datasets is not an array
  ...
  // Ensure datasets is an array before calling map
  const earningsData = Array.isArray(datasets)
    ? datasets.map((dataset) => ({
        name: dataset.title.substring(0, 20) + "...",
        earnings: dataset.price * dataset.purchases,
      }))
    : [] // Default to an empty array if datasets is not an array
                ...
                <TableBody>
                  {datasets.map((dataset) => (
                    <TableRow key={dataset._id}>
                      ...
                      <TableCell>{dataset.purchases}</TableCell>
                      <TableCell>
